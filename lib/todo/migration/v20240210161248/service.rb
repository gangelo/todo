  ...
  module Migration
    module V20240210161248
      class Service < BaseService
        ...
        def todo_folder_from
          File.join(root_folder, 'todo')
        end

        def entries_folder_from
          File.join(todo_folder_from, 'entries')
        end

        def themes_folder_from
          File.join(todo_folder_from, 'themes')
        end
        ...
        def add_new_color_themes
          ...
          %w[light.json christmas.json].each do |theme_file|
            ...
            # todo version with bugs. We need to overwrite them with this new version.
        ...
        def create_current_project_file
          ...
          # NOTE: todo_folder won't change and is safe to use here.
          FileUtils.cp(File.join(seed_data_folder, 'current_project.json'), todo_folder)
        end
        ...
        def update_entry_groups
          ...
          entries_folder_to = File.join(todo_folder, 'projects', default_project_name, 'entries')
        ...
        def update_color_themes
          ...
          themes_folder_to = File.join(todo_folder, 'themes')
