...
require_relative '../support/command_options/todo_times'
...
module Todo
  module Subcommands
    class List < BaseSubcommand
      ...
      def today
        time = Time.now
        times = sorted_todo_times_for(times: [time.yesterday, time])
      ...
      def tomorrow
        time = Time.now
        times = sorted_todo_times_for(times: [time, time.tomorrow])
        presenter = Presenters::EntryGroup::List::DatePresenter.new(times: times, options: options)
        Views::EntryGroup::List.new(presenter: presenter).render
      end
      ...
      def yesterday
        time = Time.now
        times = sorted_todo_times_for(times: [time.yesterday, time.yesterday.yesterday])
        presenter = Presenters::EntryGroup::List::DatePresenter.new(times: times, options: options)
        Views::EntryGroup::List.new(presenter: presenter).render
      end
      ...
      def date(date_or_mnemonic)
        ...
        times = sorted_todo_times_for(times: [time, time.yesterday])
        presenter = Presenters::EntryGroup::List::DatePresenter.new(times: times, options: options)
        Views::EntryGroup::List.new(presenter: presenter).render
      ...
      def dates
        ...
        times, errors = Support::CommandOptions::TodoTimes.todo_times_for(from_option: options[:from], to_option: options[:to]) # rubocop:disable Layout/LineLength
        if errors.any?
