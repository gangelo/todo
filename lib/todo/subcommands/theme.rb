...
module Todo
  module Subcommands
    class Theme < BaseSubcommand
      # TODO: I18n.
      map %w[c] => :create if Todo.env.local?
      map %w[d] => :delete if Todo.env.local?
      map %w[l] => :list
      map %w[s] => :show
      map %w[u] => :use
      ...
      def use(theme_name = Models::ColorTheme::DEFAULT_THEME_NAME)
        ...
        return if Todo.env.local? && !Models::ColorTheme.exist?(theme_name: theme_name) && !create(theme_name)
      ...
      def show(theme_name = configuration.theme_name)
        if Todo::Models::ColorTheme.exist?(theme_name: theme_name)
      ...
      def display_todo_header
        self.class.display_todo_header
      end
