    ...
    module CommandHookable
      ...
      module ClassMethods
        def start(args = ARGV, options = {})
          display_todo_header unless suspend_header?(args, options)
        ...
        def display_todo_header
          if Todo.env.screen_shot_mode?
            puts apply_theme('Running screen shot mode!', theme_color: color_theme.warning)
            puts "#{Todo.env.screen_shot_prompt} todo #{ARGV.join(' ')}"
          end
        end

        def display_todo_footer
          puts apply_theme('_' * 50, theme_color: color_theme.todo_footer)
          # TODO: I18n.
          puts apply_theme("todo v#{Todo::VERSION} | Project: #{project} | Theme: #{color_theme.theme_name}",
            theme_color: color_theme.todo_footer)
        end
