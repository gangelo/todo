  ...
  module Views
    module ColorTheme
      ...
      class Show
        ...
        def presenter
          @presenter ||= Todo::Presenters::ColorThemeShowPresenter.new(color_theme, options: options)
        end
        ...
        def display_entry_group_example
          puts apply_theme('`todo list` example', theme_color: color_theme.subheader)
          puts
        ...
        def display_configuration_example
          puts apply_theme('`todo config info` example', theme_color: color_theme.subheader)
          puts
        ...
        def mock_entry_group(options)
          @mock_entry_group ||= Todo::Models::EntryGroup.new(time: Time.now, entries: [
            Todo::Models::Entry.new(description: 'Todo entry 1', options: options),
            Todo::Models::Entry.new(description: 'Todo entry 2', options: options),
            Todo::Models::Entry.new(description: 'Todo entry 3', options: options)
          ], options: options)
        end
