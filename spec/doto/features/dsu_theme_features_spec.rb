...
RSpec.describe 'Doto theme features', type: :feature do
  ...
  shared_examples 'the color theme exists' do
    it 'creates the color theme' do
      cli
      expect(Doto::Models::ColorTheme.exist?(theme_name: theme_name)).to be(true)
    end
  end
  ...
  shared_examples 'the color theme does not exist' do
    it 'does not create the color theme' do
      cli
      expect(Doto::Models::ColorTheme.exist?(theme_name: theme_name)).to be(false)
    end
  end
  ...
  describe '#delete' do
    context 'when the color theme is the default theme' do
      let(:theme_name) { Doto::Models::ColorTheme::DEFAULT_THEME_NAME }
    ...
    context 'when the color theme exists' do
      before do
        Doto::Models::ColorTheme.find_or_create(theme_name: theme_name)
      end
  ...
  describe '#list' do
    ...
    context 'when the default color theme does not exist' do
      before do
        theme_names.each { |theme_name| create(:color_theme, theme_name: theme_name) }
        allow(Doto::Models::ColorTheme).to receive(:exist?).with(theme_name: Doto::Models::ColorTheme::DEFAULT_THEME_NAME).and_return(false)
      end
  ...
  describe '#use' do
    context 'when no color theme is specified' do
      ...
      it 'uses the current color theme' do
        default_theme_name = Doto::Models::ColorTheme::DEFAULT_THEME_NAME
    ...
    context 'when the color theme file does not exist' do
      context 'when not running locally and the color theme does not exist' do
        before do
          allow(Doto.env).to receive(:local?).and_return(false)
        end
...
def color_theme_regex_for(theme_names:, default_theme_name: Doto::Models::ColorTheme::DEFAULT_THEME_NAME)
