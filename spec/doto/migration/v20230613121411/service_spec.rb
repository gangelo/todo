...
RSpec.describe Doto::Migration::V20230613121411::Service, type: :migration do
  ...
  let(:doto_folder) { File.join(temp_folder, 'doto') }
  let(:options) { {} }
  ...
  describe '#migrate_if!' do
    ...
    context 'when the migration version is 0' do
      ...
      context 'when the pretend option is set to false' do
        ...
        it 'migrates the doto folder structure and files' do
          expect(doto_folders_and_file_contents_match?(expected: expected, actual: actual,
