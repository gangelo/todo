...
RSpec.describe Doto::Models::EntryGroup do
  subject(:entry_group) { build(:entry_group, time: time, entries: entries) }

  before do
    build(:configuration, config_hash: Doto::Models::Configuration::DEFAULT_CONFIGURATION)
  end
  ...
  describe '#hash' do
    let(:expected_hash) do
      entry_group.entries.map(&:hash).tap do |hashes|
        ...
        hashes << Doto::Support::TimeComparable.time_equal_compare_string_for(time: entry_group.time)
      end.hash
    end
  ...
  describe 'validation' do
    it 'validates #version attribute with the VersionValidator' do
      expect(described_class).to validate_with_validator(Doto::Validators::VersionValidator)
    end

    it 'validates #entries attribute with the EntriesValidator' do
      expect(described_class).to validate_with_validator(Doto::Validators::EntriesValidator)
    end

    it 'validates #time with TimeValidator' do
      expect(described_class).to validate_with_validator(Doto::Validators::TimeValidator)
    end
  ...
  describe 'class methods' do
    ...
    describe '.delete' do
      context 'when an entry group file exists for :time' do
        ...
        it 'deletes the file' do
          ...
          entry_path = Doto::Support::Fileable.entries_path(time: time)
    ...
    describe '.edit' do
      before do
        allow(Doto::Services::StdoutRedirectorService).to receive(:call).and_return(tmp_file_contents)
      ...
      let(:tmp_file_contents) do
        ...
        Doto::Views::EntryGroup::Edit.new(entry_group: changed_entry_group).render_as_string
      end
