...
RSpec.describe Doto::Models::Project do
  subject(:project) do
  ...
  describe 'validations' do
    ...
    it 'validates #description attribute with the DescriptionValidator' do
      expect(described_class).to validate_with_validator(Doto::Validators::DescriptionValidator)
    end

    it 'validates #description attribute with the ProjectNameValidator' do
      expect(described_class).to validate_with_validator(Doto::Validators::ProjectNameValidator)
    end
    ...
    context 'when project_name is greater than the min/max length' do
      let(:project_name) { 'x' * (1 + Doto::Models::Project::MAX_PROJECT_NAME_LENGTH) }
    ...
    context 'when description is greater than the min/max length' do
      let(:description) { 'x' * (1 + Doto::Models::Project::MAX_DESCRIPTION_LENGTH) }
  ...
  describe '#project_file' do
    it 'returns the project file' do
      expected_project_file = Doto::Support::Fileable.project_file_for(project_name: project.project_name)
  ...
  describe '#project_folder' do
    it 'returns the project folder' do
      expected_project_folder = Doto::Support::Fileable.project_folder_for(project_name: project.project_name)
