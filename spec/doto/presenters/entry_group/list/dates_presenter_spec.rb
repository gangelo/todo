...
RSpec.describe Doto::Presenters::EntryGroup::List::DatesPresenter do
  ...
  describe '#render' do
    subject(:presenter) do
      strip_escapes(Doto::Services::StdoutRedirectorService.call do
  ...
  describe '#display_nothing_to_list_message' do
    ...
    context 'when the entry groups do not exist for the times and the include_all option is false' do
      let(:expected_message) do
        Doto::Views::EntryGroup::Shared::NoEntriesToDisplay.new(times: times, options: options).render_as_string
      end
