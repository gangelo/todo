...
RSpec.describe Doto::Services::EntryGroup::DeleterService do
  ...
  describe '#call' do
    ...
    context 'when there are entry groups for the times given' do
      ...
      it do
        expect(Doto::Services::EntryGroup::CounterService.new(times: times).call).to be times.count
      end

      it 'deletes the entry groups' do
        service.call
        expect(Doto::Services::EntryGroup::CounterService.new(times: times).call).to be_zero
      end
