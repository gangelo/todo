...
RSpec.describe Doto::Services::EntryGroup::EditorService do
  ...
  describe '#call' do
    ...
    context 'when there are no changes' do
      ...
      let(:tmp_file_contents) do
        Doto::Views::EntryGroup::Edit.new(entry_group: original_entry_group).render_as_string
      end
      ...
      it 'does not change the entry group file' do
        expect(Doto::Models::EntryGroup.find(time: time)).to eq(entry_group)
      end
    end
    ...
    context 'when all the entries are deleted' do
      ...
      it 'deletes the entry group file' do
        expect(Doto::Models::EntryGroup.exist?(time: original_entry_group.time)).to be false
      end
    end
    ...
    context 'when entries are added' do
      ...
      it 'saves the changes to the entry group file' do
        expect(Doto::Models::EntryGroup.find(time: time)).to eq(changed_entry_group)
      end
    end
    ...
    context 'when the entry group file is edited incorrectly' do
      context 'when the entry group fails validation' do
        ...
        it 'saves all the valid entries to the entry group file and ignores the invalid entries' do
          ...
          expect(Doto::Models::EntryGroup.find(time: time)).to eq(expected_entry_group)
        end
      end
    end
  end
