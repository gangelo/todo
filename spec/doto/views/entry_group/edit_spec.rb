...
RSpec.describe Doto::Views::EntryGroup::Edit do
  subject(:edit_view) { described_class.new(entry_group: entry_group, options: options) }

  after do
    Doto::Models::EntryGroup.delete(time: today)
    Doto::Models::EntryGroup.delete(time: yesterday)
  end
  ...
  context 'when there are entries for today and previous entries' do
    ...
    it 'has entries for yesterday on disk' do
      expect(Doto::Models::EntryGroup.exist?(time: yesterday)).to be true
    end
