...
RSpec.describe Todo::CommandServices::AddEntryService do
  ...
  describe '#call' do
    ...
    context 'when an entry fails validation' do
      ...
      it 'does not add the entry' do
        ...
        expect(Todo::Models::EntryGroup.exist?(time: time)).to_not be true
      end
    end
    ...
    context 'when the entry is added' do
      ...
      it 'creates the entry group file' do
        expect(Todo::Models::EntryGroup.exist?(time: time)).to be true
      end

      it 'creates the entry group file with the correct json' do
        expected_entry_group = Todo::Models::EntryGroup.new(time: time, entries: [entry])
