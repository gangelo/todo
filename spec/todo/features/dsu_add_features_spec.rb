...
RSpec.describe 'Todo add features', type: :feature do
  subject(:cli) do
    capture_stdout_and_strip_escapes { Todo::CLI.start(args) }
  end
  ...
  context "when 'todo help add' is called" do
    let(:args) { %w[help add] }
  ...
  context "when 'todo add DESCRIPTION' is called" do
    before do
    ...
    let(:expected_date) do
      Todo::Support::TimeFormatable.formatted_time(time: entry_date)
    end
  ...
  context "when 'todo add --tomorrow' is called" do
    before do
      with_entries
    ...
    let(:expected_date) do
      Todo::Support::TimeFormatable.formatted_time(time: Time.now.tomorrow)
    end
  ...
  context "when 'todo add --yesterday' is called" do
    before do
      with_entries
    ...
    let(:entry_description) { 'This is a test' }
    let(:expected_date) do
      Todo::Support::TimeFormatable.formatted_time(time: Time.now.yesterday)
    end
  ...
  context "when 'todo add --today' is called" do
    before do
      with_entries
    ...
    let(:entry_description) { 'This is a test' }
    let(:expected_date) do
      Todo::Support::TimeFormatable.formatted_time(time: Time.now)
    end
  ...
  context "when 'todo add --date=DATE' is called" do
    before do
      with_entries
    end
    ...
    context 'when using option --date' do
      ...
      let(:entry_description) { 'This is a test' }
      let(:expected_date) do
        Todo::Support::TimeFormatable.formatted_time(time: Time.parse(entry_date))
      end
    ...
    context 'when an error is raised' do
      subject(:cli) do
        capture_stderr_and_strip_escapes { Todo::CLI.start(args) }
      end

      before do
        allow(Todo::Models::Entry).to receive(:new).and_raise(ArgumentError, 'Boom!')
      end
