...
RSpec.describe 'Todo browse features', type: :feature do
  subject(:cli) do
    strip_escapes(Todo::Services::StdoutRedirectorService.call { Todo::CLI.start(args) })
  end
  ...
  context "when 'todo help browse' is used" do
    let(:args) { %w[help browse] }

    it_behaves_like 'help is displayed'
  end

  context "when 'todo browse COMMAND' is used" do
    ...
    context "with 'week'" do
      ...
      it 'displays the entry groups for the week' do
        entry_groups.each do |entry_group|
          entry_group_header = Todo::Support::TimeFormatable.formatted_time(time: entry_group.time)
    ...
    context "with 'week' and no entry groups and option :include_all is true" do
      ...
      it 'displays the dates for the week with no entry groups' do
        nothing_to_display_header = Todo::Views::EntryGroup::Shared::NoEntriesToDisplayForWeekOf.new(time: week_of).render_as_string
    ...
    context "with 'month' and no entry groups and option :include_all is true" do
      ...
      it 'displays the dates for the month with no entry groups' do
        nothing_to_display_header = Todo::Views::EntryGroup::Shared::NoEntriesToDisplayForMonthOf.new(time: month_of).render_as_string
        expect(cli).to include(strip_escapes(nothing_to_display_header))
      end
    end
    ...
    context "with 'year' and no entry groups and option :include_all is true" do
      ...
      it 'displays the dates for the year with no entry groups' do
        nothing_to_display_header = Todo::Views::EntryGroup::Shared::NoEntriesToDisplayForYearOf.new(time: year_of).render_as_string
        expect(cli).to include(strip_escapes(nothing_to_display_header))
      end
    end
  end
