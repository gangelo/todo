...
RSpec.describe 'Todo list features', type: :feature do
  ...
  context "when 'todo help list' is used" do
  ...
  context "when 'todo list date DATE|MNEMONIC' is used" do
    ...
    context 'with a date' do
      let(:args) { ['list', 'date', Todo::Support::TimeFormatable.yyyy_mm_dd(time: time, separator: '/')] }
  ...
  context "when 'todo list dates OPTIONS' is used" do
      ...
      context 'with invalid FROM_DATE' do
        let(:args) do
          [
            ...
            'dates',
            '--from', 'bad-date',
            '--to', Todo::Support::TimeFormatable.mm_dd(time: Time.now)
          ]
        end
        let(:expected_output) do
      ...
      context 'with invalid TO_DATE' do
        let(:args) do
          [
            'list',
            'dates',
            '--from', Todo::Support::TimeFormatable.mm_dd(time: Time.now),
            '--to', 'bad-date'
          ]
        end
        let(:expected_output) do
    ...
    context "when 'todo list dates -f FROM_DATE -t TO_DATE' is used" do
      let(:args) do
        [
          'list',
          'dates',
          '-f', Todo::Support::TimeFormatable.mm_dd_yyyy(time: times.min),
          '-t', Todo::Support::TimeFormatable.mm_dd_yyyy(time: times.max)
        ]
      end
      let(:times) { [Time.now.yesterday, Time.now] }
    ...
    context "when 'todo list dates -f MNEMONIC -t TO_DATE' is used" do
      let(:args) do
        [
          'list',
          'dates',
          '-f', 'yesterday',
          '-t', Todo::Support::TimeFormatable.mm_dd(time: Time.now)
        ]
      end
      let(:times) { [Time.now.yesterday, Time.now] }
    ...
    context "when 'todo list dates -f FROM_DATE -t MNEMONIC' is used" do
      let(:args) do
        [
          'list',
          'dates',
          '-f', Todo::Support::TimeFormatable.mm_dd_yyyy(time: Time.now.yesterday),
          '-t', 'today'
        ]
      end
      let(:times) { [Time.now.yesterday, Time.now] }
    ...
    context "when 'todo list dates -f MNEMONIC -t MNEMONIC' is used" do
...
def view_entry_groups(times)
  ...
  times = todo_times_for(times)
  times = Todo::Support::TimesSortable.times_sort(times: times, entries_display_order: config_hash[:entries_display_order])
  presenter = Todo::Presenters::EntryGroup::List::DatesPresenter.new(times: times, options: config_hash)
  Todo::Views::EntryGroup::List.new(presenter: presenter).render
end

def todo_times_for(times)
  from = Todo::Support::TimeFormatable.mm_dd_yyyy(time: times.min)
  to = Todo::Support::TimeFormatable.mm_dd_yyyy(time: times.max)
  times, errors = Todo::Support::CommandOptions::TodoTimes.todo_times_for(from_option: from, to_option: to)
  raise errors.join("\n") if errors.any?
