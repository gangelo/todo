...
RSpec.describe Todo::Migration::BaseService, type: :migration do
  ...
  context 'when the required methods are implemented' do
    ...
    let(:to_migration_version) { Todo::Migration::VERSION }
    ...
    describe 'class methods' do
      ...
      describe '.migrates_to_latest_migration_version?' do
        context 'when the to migration version is not the latest' do
          let(:from_migration_version) { 1 }
          let(:to_migration_version) { 1 + Todo::Migration::VERSION }
    ...
    describe 'instance methods' do
      describe '#migrate_if!' do
        before do
          stub_const('Todo::Migration::VERSION', to_migration_version)
        ...
        context 'when the current migration version is equal to the from migration version' do
          ...
          it 'creates a backup of the current todo folder and updates the migration version' do
            expected_backup_folder = Todo::Support::Fileable.backup_folder_for(migration_version: migration_version)
            expect(Dir.exist?(expected_backup_folder)).to be true
            expect(Todo::Models::MigrationVersion.new.version).to eq to_migration_version
          end
        end
        ...
        context 'when the current migration version is not equal to the from migration version' do
          ...
          it 'does not create a backup of the current todo folder and does not update the migration version' do
            expected_backup_folder = Todo::Support::Fileable.backup_folder_for(migration_version: from_migration_version)
            expect(Dir.exist?(expected_backup_folder)).to be false
            expect(Todo::Models::MigrationVersion.new.version).to_not eq from_migration_version
          end
        end
      end
    end
