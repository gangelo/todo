...
RSpec.describe Todo::Migration::Migrator, type: :migration do
  before do
  ...
  let(:v20230613121411_migration_service) { Todo::Migration::V20230613121411::Service.new(options: options) }
  let(:v20240210161248_migration_service) { Todo::Migration::V20240210161248::Service.new(options: options) }
  ...
  describe '.migrate_if!' do
    ...
    context 'when there is no migration service to migrate to the latest migration version' do
      ...
      let(:expected_error) { "No migration service handles the current migration: #{Todo::Migration::VERSION}." }
