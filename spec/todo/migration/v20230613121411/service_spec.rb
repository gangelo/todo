...
RSpec.describe Todo::Migration::V20230613121411::Service, type: :migration do
  ...
  let(:todo_folder) { File.join(temp_folder, 'todo') }
  let(:options) { {} }
  ...
  describe '#migrate_if!' do
    ...
    context 'when the migration version is not 0' do
      ...
      let(:actual) { todo_folder }

      it 'does not make any changes to the todo folder structure or configuration file' do
        expect(todo_folders_and_file_contents_match?(expected: expected, actual: actual)).to be(true)
      end
    end
    ...
    context 'when the migration version is 0' do
      ...
      context 'when the pretend option is set to false' do
        ...
        it 'migrates the todo folder structure and files' do
          expect(todo_folders_and_file_contents_match?(expected: expected, actual: actual,
