...
RSpec.describe Todo::Models::ColorTheme do
  ...
  shared_examples 'the color theme exists' do
    it 'the color theme file exists' do
      theme_path = Todo::Support::Fileable.themes_path(theme_name: theme_name)
  ...
  describe 'validations' do
    it 'validates #description with DescriptionValidator' do
      expect(color_theme).to validate_with_validator(Todo::Validators::DescriptionValidator)
    end

    it 'validates the color theme color attributes with the ColorThemeValidator' do
      expect(color_theme).to validate_with_validator(Todo::Validators::ColorThemeValidator)
    end

    it 'validates #version attribute with the VersionValidator' do
      expect(color_theme).to validate_with_validator(Todo::Validators::VersionValidator)
    end
  end
      ...
      let(:expected_default_theme_hash) do
        {
          ...
          help: { color: :cyan },
          todo_header: { color: :white, mode: :bold, background: :cyan },
          todo_footer: { color: :cyan },
  ...
  describe 'class methods' do
    describe '.all' do
      context 'when there are no color themes' do
        before do
          themes_folder = Todo::Support::Fileable.themes_folder
