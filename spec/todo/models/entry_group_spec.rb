...
RSpec.describe Todo::Models::EntryGroup do
  subject(:entry_group) { build(:entry_group, time: time, entries: entries) }

  before do
    build(:configuration, config_hash: Todo::Models::Configuration::DEFAULT_CONFIGURATION)
  end
  ...
  describe '#hash' do
    let(:expected_hash) do
      entry_group.entries.map(&:hash).tap do |hashes|
        ...
        hashes << Todo::Support::TimeComparable.time_equal_compare_string_for(time: entry_group.time)
      end.hash
    end
  ...
  describe 'validation' do
    it 'validates #version attribute with the VersionValidator' do
      expect(described_class).to validate_with_validator(Todo::Validators::VersionValidator)
    end

    it 'validates #entries attribute with the EntriesValidator' do
      expect(described_class).to validate_with_validator(Todo::Validators::EntriesValidator)
    end

    it 'validates #time with TimeValidator' do
      expect(described_class).to validate_with_validator(Todo::Validators::TimeValidator)
    end
  ...
  describe 'class methods' do
    ...
    describe '.delete' do
      context 'when an entry group file exists for :time' do
        ...
        it 'deletes the file' do
          ...
          entry_path = Todo::Support::Fileable.entries_path(time: time)
    ...
    describe '.edit' do
      before do
        allow(Todo::Services::StdoutRedirectorService).to receive(:call).and_return(tmp_file_contents)
      ...
      let(:tmp_file_contents) do
        ...
        Todo::Views::EntryGroup::Edit.new(entry_group: changed_entry_group).render_as_string
      end
