...
RSpec.describe Todo::Services::EntryGroup::DeleterService do
  ...
  describe '#call' do
    ...
    context 'when there are entry groups for the times given' do
      ...
      it do
        expect(Todo::Services::EntryGroup::CounterService.new(times: times).call).to be times.count
      end

      it 'deletes the entry groups' do
        service.call
        expect(Todo::Services::EntryGroup::CounterService.new(times: times).call).to be_zero
      end
