...
RSpec.describe Todo::Services::EntryGroup::ImporterService do
  ...
  describe '#call' do
    context 'when importing multiple entry groups that are valid and not duplicate' do
      ...
      context 'when the project name matches the current project name' do
        ...
        it 'imports the first entry group and all of the entries' do
          ...
          expect(Todo::Models::EntryGroup.find(time: times.min) == expected_entry_group).to be true
        end
        ...
        it 'imports the rest of the entry groups and all of their entries' do
          ...
          expect(Todo::Models::EntryGroup.find(time: times.max) == expected_entry_group).to be true
        end
    ...
    context 'when importing multiple entry groups with invalid entries' do
      ...
      let(:project_entry_group_entries) do
        [
          build(:entry, description: "imported_#{entry_groups[0].entries.first.description}_#{'x' * Todo::Models::Entry::MAX_DESCRIPTION_LENGTH}"),
          build(:entry, description: "imported_#{entry_groups[1].entries.first.description}_#{'y' * Todo::Models::Entry::MAX_DESCRIPTION_LENGTH}")
        ]
      end
    ...
    context 'when importing a project that is not the current project' do
      ...
      context 'when the override option is true' do
        ...
        it 'imports the entry groups and all of the entries into the current project' do
          expect(Todo::Models::EntryGroup.find(time: times.min).entries.map(&:description)).to match_array(entry_groups[0].entries.map(&:description))
          expect(Todo::Models::EntryGroup.find(time: times.max).entries.map(&:description)).to match_array(entry_groups[1].entries.map(&:description))
        end
