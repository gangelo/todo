...
RSpec.describe Todo::Services::Project::RenameService do
  ...
  describe '#call' do
    ...
    context 'when the "from project" has entry groups' do
      ...
      it 'has entry groups' do
        expect(Todo::Models::EntryGroup.find(time: entry_group_time).entries.count).to eq(2)
      end
    ...
    context 'when the "from project" does not have entry groups' do
      ...
      it 'has entry groups' do
        expect(Todo::Models::EntryGroup.all.count).to be_zero
      end
