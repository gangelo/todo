...
RSpec.describe Todo::Validators::ColorThemeValidator do
  subject(:color_theme_validator) do
    Todo::Models::ColorTheme.new(theme_name: theme_name, theme_hash: theme_hash)
  end
  ...
  let(:theme_name) { 'test' }
  let(:theme_hash) { Todo::Models::ColorTheme::DEFAULT_THEME.dup }
  ...
  context 'when there are extra color theme key/value pairs' do
    let(:theme_hash) do
      Todo::Models::ColorTheme::DEFAULT_THEME.dup.tap do |hash|
        hash[:extra] = 'extra'
      end
    end
